...
In order to check the applicability of standard turbulence models for varying viscosity flows, an analysis using \(k-\tau\) model was performed for Case I. An 8th polynomial order simulation was performed, special attention is given for the boundary conditions of \(\tau\), the treatment provided by Tomboulidesa et al. in Ref.~\cite{Tomboulidesa2018} is carried out. Fig.~\ref{fig:Re_RANS} presents the fiction Reynolds number resulted from this simulation.
...
Finally, we will compute budget terms for the Reynolds stress tensor with the objective of providing a stronger benchmark basis for RANS models.  In fact, results presented here call into question the application of standard turbulence models in cases where viscosity changes significantly over the domain (i.e., a two-fold reduction). The availability of budgett terms for the eynolds stress tensor will be the starting point to assess in more depth multiple RANS models and eventually suggest modifications if needed.
